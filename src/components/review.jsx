    ...
    <div className="form-group">
      ...
      <div>
        ...
        {this.state.reviews.map((option)=>(
          ...
          <li className="list-group-item review-card" key={option._id}><b>{option.movieName}</b><br/>{<RateShow number={option.rating}/>}<br/>~{option.userName}<br/>{option.review}</li>
        ))}
        </ul></div>
        </div>
    </div>
